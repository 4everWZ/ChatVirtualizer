...
- Session-controller tests must cover native `Edit message` transitions and confirm virtualization suspends during edit mode, restores preserved history when ChatGPT only re-renders the visible slice, and stays suspended when post-edit DOM is still too partial to trust.
