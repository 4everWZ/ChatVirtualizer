...
4. The session controller maintains both a window-mode state machine (`auto` vs `manual-expanded`) and an internal runtime phase (`bootstrapping` vs `steady`) so early pressure relief can happen before the longer steady-state debounce.
5. The virtualization engine snapshots eligible historical records, keeps detached DOM only for a short same-session retention window, and manages three render modes inside the reading flow: `live`, `lite`, and `collapsed`.
...
- Virtualization boundary: collapsed-group, live/lite promotion, and restore operations only mutate record-owned roots.
